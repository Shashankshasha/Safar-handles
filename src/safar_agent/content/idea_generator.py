...
def generate_post_copy(
    ...
    return PROVIDERS[provider](fragrance, theme, bottle_design)


def generate_post_copy_all(fragrance: Fragrance, theme: Theme, bottle_design: str) -> dict:
    """Runs every configured provider and returns {provider_name: result_or_error}."""
    results: dict[str, dict] = {}
    for name, generate in PROVIDERS.items():
        try:
            results[name] = generate(fragrance, theme, bottle_design)
